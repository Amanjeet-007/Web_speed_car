...
function buildWhatsAppMessage(data) {
  const itemLines = data.items
    .map(
      (item) =>
        `• ${item.title} (${item.category}) - ${item.price && Number(item.price) > 0 ? `₹${item.price}` : 'Price on request'}`
    )
    .join("\n");
...
Payment Method: ${data.paymentMethod === 'online' ? 'Online Paid (Razorpay)' : 'Pay after Service / Cash'}
${data.paymentId ? `Payment ID: ${data.paymentId}` : ''}

${data.hasCustomPrice ? '*Total Amount: Price on request*' : `Subtotal: ₹${data.subtotal}\nGST (18%): ₹${data.tax}\n*Total Amount: ₹${data.total}*`}
...
export default function CheckoutPage() {
  ...
  // Check karein ki cart mein koi aisi service hai jiska price available nahi hai
  ...
  const handleCheckoutSubmit = async (e) => {
    ...
    // Agar Offline / COD select kiya hai ya custom price item hai
    if (formData.paymentMethod === "cod" || hasCustomPriceItems) {
      const randomRef = "SCW-" + Math.floor(100000 + Math.random() * 900000);
      setSuccessData({
        bookingReference: randomRef,
        paymentId: "OFFLINE / PAY AT WORKSHOP",
        customer: { ...formData },
        ...
        total: grandTotal,
        paymentMethod: "cod",
        hasCustomPrice: hasCustomPriceItems,
      });
      dispatch(clearCart());
      return;
    }
          ...
          >
            <div className="bg-[#071018] text-white px-6 sm:px-10 py-8 text-center">
              ...
              <p className="text-sm text-gray-400 mt-2">
                Your car wash appointment has been successfully placed.
              </p>
            </div>
            ...
            <div className="p-6 sm:p-10">
                  ...
                  {successData.items.map((item, index) => (
                    <div key={`${item.title}-${index}`} className="flex justify-between gap-4 text-sm">
                      ...
                      <span className="font-bold text-gray-900">
                        {item.price && Number(item.price) > 0 ? `₹${item.price}` : "Price on request"}
                      </span>
                    </div>
                  ))}
                </div>
              ...
              <div className="py-6 space-y-2 text-sm">
                {successData.hasCustomPrice ? (
                  <div className="flex justify-between pt-3 border-t border-gray-100 text-base font-extrabold text-gray-900">
                    <span>Total Amount</span>
                    <span className="text-amber-600 text-xs sm:text-sm uppercase tracking-wider">Price on request</span>
                  </div>
                ) : (
                  <>
        ...
        <div className="max-w-6xl mx-auto px-4">
          <div className="mb-8">
            ...
            <p className="text-sm text-gray-500 mt-1">
              Secure online payment via Razorpay or choose to pay after service.
            </p>
          </div>
                ...
                <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                  ...
                  <div className="space-y-3">
                    <label className={`flex items-center gap-3 p-3 rounded-xl border transition ${hasCustomPriceItems ? 'bg-gray-100 opacity-50 cursor-not-allowed border-gray-200' : 'bg-gray-50 cursor-pointer border-gray-200'}`}>
                      ...
                      />
                      <span className="text-xs font-semibold text-gray-800">Online Payment via Razorpay (UPI, Credit/Debit Card, NetBanking)</span>
                    </label>
                ...
                <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 sticky top-24">
                  ...
                  <div className="space-y-4 max-h-64 overflow-y-auto pr-1 mb-4">
                    {cartItems.map((item, index) => {
                      const hasValidPrice = item.price && Number(item.price) > 0;
                  ...
                  <div className="space-y-2 text-xs text-gray-600 pt-2 border-t border-gray-100">
                    {hasCustomPriceItems ? (
                      <div className="flex justify-between pt-3 border-t border-gray-100 text-sm font-bold text-gray-900">
                        <span>Total Amount</span>
                        <span className="text-amber-600 text-[11px] uppercase tracking-wider">Price on request</span>
                      </div>
                    ) : (
                      <>
                  ...
                  >
                    {processing ? "PROCESSING..." : (formData.paymentMethod === 'online' && !hasCustomPriceItems ? "CONFIRM & PAY NOW" : "CONFIRM BOOKING")}
                  </button>

                  <p className="text-[10px] text-gray-400 text-center mt-3 leading-4">
                    Speed Car Wash secure booking system.
                  </p>
                </div>
              </div>
            </form>
